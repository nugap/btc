...
console.log('\n📋 Step 5: Full air-gap flow (hodl_test sign → yolo_test finalize)');
...
if (ft.ok) {
  assert(true, 'Full flow: PSBT → base64 → sign → base64 → finalize (' + ft.rawLen + 'B)');
...
console.log('\n📋 Step 6: Yolo finalize from hodl-signed PSBT');
var yt = yWin.eval("(function() {\
  try {\
    var net = nugap.btcSigner.TEST_NETWORK;\
    return { ok: true, hasTx: typeof nugap.btcSigner.Transaction !== 'undefined' };\
  } catch(e) { return { ok: false, error: e.message }; }\
})()");
assert(yt.ok && yt.hasTx, 'yolo_test: Transaction class available');
